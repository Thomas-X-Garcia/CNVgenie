...
**CNVgenie** is a comprehensive, integrated pipeline for copy number variant (CNV) detection in long-read sequencing data aligned to telomere-to-telomere (T2T) reference genomes. This toolkit provides a complete workflow from raw BAM/CRAM files to final CNV calls, encompassing coverage analysis, baseline generation, and advanced detection algorithms specifically optimized for long-read sequencing data.

### Motivation

While existing tools for copy number variation (CNV) analysis, such as [QDNAseq](https://github.com/ccagc/QDNAseq) and Spectre ([Fritz Sedlazeck's version](https://github.com/fritzsedlazeck/Spectre), [ONT version](https://github.com/nanoporetech/ont-spectre)), offer robust performance for long-read sequencing data, they present significant limitations by being either incompatible with the gapless [T2T-CHM13v2.0](https://github.com/marbl/CHM13) reference genome or designed exclusively for large-scale variations exceeding 100,000 base pairs. This creates a critical analytical gap, particularly for complex regions like the Y chromosome, which contains 57% ambiguous represention in the GRCh38 reference. To address this, CNVgenie was developed to leverage a panel of normals to establish a precise baseline read depth, enabling the accurate detection of CNVs as small as 15 kilobase pairs. Although developed with intent for T2T usage, CNVgenie’s flexible architecture is compatible with any reference genome, offering a versatile and powerful solution for high-resolution CNV discovery across diverse genomic contexts.
